...
public class ScheduledPredictionService {
    ...
    private void processUser(User user, LocalDate today) throws Exception {
        // Look back to the start of the current ovulation window — no point confirming older than that
        ...
        // All predicted period first-days from ovulation window start up to today
        List<EventData> candidates = eventDataRepository.findPredictedPeriodFirstDaysBetween(
            user.getId(), lookbackStart, today);
        if (candidates.isEmpty()) return;

        log.info("User {} — checking {} predicted period date(s) back to {}", user.getEmail(), candidates.size(), lookbackStart);

        Calendar userCalendar = calendarClientFactory.buildForUser(user.getRefreshToken());

        for (EventData predicted : candidates) {
            LocalDate date = predicted.getEventDate();
            String volcanoEventId = findVolcanoEvent(userCalendar, date);
            if (volcanoEventId == null) {
                log.info("No 🌋🔥 on {} for user {}", date, user.getEmail());
                continue;
            }
            ...
            // Regenerate predictions and stop — one confirmed period per run is enough
            try {
